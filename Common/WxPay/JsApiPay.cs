...
namespace Common.WxPay
{
    public class JsApiPay
    {
        ...
        */
        ...
        public string WxCodeUrl(string redirectUri, string state)
        {
            string host = page.Request.Url.Host;
            string redirect_uri = HttpUtility.UrlEncode("http://" + host + redirectUri);
	    ...
	    */
        ...
        public static void SendMsg(string UserOpenid, string MsgTemplate, object Data, string ReturnUrl = "")
        {
            try
            {
                ...
                if (((System.Collections.IDictionary)jd).Contains("access_token"))
                {
                    ...
                    // 发送模板消息
                    SendMsgPoth(ConvertTools._ConvertTools.SerializeObject(msgData), access_token);
                }
                else {
                    MYLog.Debug("发送消息通知：" + SessionHelper._SessionHelper.UserName, "获取AccessToken结果：" + resultStr);
                }
            }
            catch (Exception ex)
            {
                MYLog.Error("发送消息通知：" + SessionHelper._SessionHelper.UserName, ex.ToString());
            }
        }
        ...
        public static void SendMsgPoth(string jsonData, string access_token)
        {
            ...
            {
                MYLog.Error("发送消息通知失败：" + SessionHelper._SessionHelper.UserName, (string)jd["errmsg"]);
            }
        }
    }
}
